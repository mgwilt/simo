...
- Dirty paths: the uncommitted Breeze sidecar, runtime/profile integration, LAN site, tests, dependencies, source submodule, and documentation updates described by `E-007`.
...
- Active `T-010` implementation: Breeze is the default TTS backend; official source `0072588…` and model `799624c…` are pinned; a loopback-only sidecar uses eager PyTorch MPS without changing vendored source; runtime-profile v2 captures voice design, instruction, CFG scale, and seed while v1 profiles resolve to Qwen rollback.
