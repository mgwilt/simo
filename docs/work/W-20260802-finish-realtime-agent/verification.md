...
- `E-011` verifies `A-012` at `6907ee5`: the installed pre-commit launcher invokes Lefthook, the explicit all-files route passes frozen Ruff `ALL` lint and formatting, `ty` with warning failure, and baseline-ratcheted BasedPyright strict. The pre-push contract passes the native build, 65 Python tests, documentation validation, and five knowledge tests. Managed `.git` permissions blocked replacement of existing launcher files, not invocation.
...
- Cancellation during an in-flight Metal kernel and repeated per-turn p95 latency remain unverified. Subjective headset and room quality are explicitly outside synthetic attestation. The terminal closure satisfies `A-011` with local publication references and no claim of a remote push or deployment.
